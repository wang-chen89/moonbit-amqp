...
try {
  ...
  if (command === '--help') {
    console.log('Usage: node tools/broker.mjs roundtrip | publish QUEUE | get QUEUE\nConfiguration: AMQP_HOST, AMQP_PORT, AMQP_USER, AMQP_PASSWORD, AMQP_VHOST. AMQP_TLS=1 enables TLS; AMQP_CA points to a trusted CA PEM file. TCP requires AMQP_ALLOW_INSECURE=1. Publish reads up to 1 MiB from stdin and waits for a publisher confirm. Get prints JSON with bodyBase64 and then acknowledges; empty queue prints null. Roundtrip uses an exclusive temporary queue. Exit 0 success, 1 error.');
  } else {
    ...
    connection=await connect({host:process.env.AMQP_HOST??'localhost',port:Number(process.env.AMQP_PORT??(secure?5671:5672)),username:process.env.AMQP_USER??'guest',password:process.env.AMQP_PASSWORD??'guest',vhost:process.env.AMQP_VHOST??'/',allowInsecureAuth:process.env.AMQP_ALLOW_INSECURE==='1',tls:secure?{...(process.env.AMQP_CA?{ca:fs.readFileSync(process.env.AMQP_CA)}:{})}:undefined});
