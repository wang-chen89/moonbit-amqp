...
import {fixture,delay,method,u16,u32,short,deliver,until} from './recovery-peer.mjs';
...
await test('no-op policy receives both connection and channel loss without automatic redial',{},async({open,state})=>{
 const seen=[],ended=gate(),s=noop();s.onConnectionClose=(c,error)=>{seen.push(['connection',error.message]);ended.release();};s.onChannelClose=(ch,error)=>seen.push(['channel',error.message]);const c=await open(config(s)),ch=await c.openChannel();await ch.declareQueue('retained');[...state.sockets][0].destroy();await ended.promise;await until(()=>seen.length===2);assert.equal(state.accepted,1);assert.equal(c.state,'disconnected');assert.equal(ch.state,'disconnected');assert(c._raw.closed&&ch._raw.closed);assert(!c.closed&&!ch.closed);assert(c.topologyConfiguration().queues.retained);assert.equal(await closedSignal(c.notifyRecoveryCancel()),false);assert.equal(await closedSignal(ch.notifyRecoveryCancel()),false);assert.deepEqual(seen.map(x=>x[0]).sort(),['channel','connection']);await assert.rejects(c.openChannel());await assert.rejects(ch.get('retained'));
});
await test('manual reconnect after a no-op decision restores the original channel and consumer',{},async({open,state})=>{
 const ended=gate(),s=noop();s.onConnectionClose=()=>ended.release();const c=await open(config(s)),ch=await c.openChannel(),messages=[];await ch.declareQueue('manual');await ch.consume('manual',m=>messages.push(m.body.toString()),{consumerTag:'manual',noAck:true});[...state.sockets][0].destroy();await ended.promise;await Promise.all([c.reconnect(),ch.reconnect(),c.reconnect()]);assert.equal(state.accepted,2);assert.equal(ch.state,'open');deliver([...state.sockets].at(-1),ch._raw.id,1,'manual','restored');await until(()=>messages.length===1);assert.deepEqual(messages,['restored']);
});
...
await test('completed no-op contexts expire while the public manual API remains usable',{},async({open,state})=>{
 let ctx;const entered=gate(),s=noop();s.onConnectionClose=(c,e,x)=>{ctx=x;entered.release();};const c=await open(config(s));[...state.sockets][0].destroy();await entered.promise;await until(()=>ctx.signal.aborted);assert(ctx.signal.aborted);await assert.rejects(ctx.reconnect(),expired);await c.reconnect();assert.equal(c.state,'open');
});
...
await test('all sixteen pending channel decisions cancel without runtime warnings', {onMethod(e,s){if(e.cls===10&&e.id===11){s.write(method(0,10,30,u16(64),u32(8192),u16(0)));return true;}}},async({open,state})=>{
 ...
 try{const entered=gate(),contexts=[];const s={onConnectionClose(c,e,x){contexts.push(x);entered.release();return new Promise(()=>{});},onChannelClose(ch,e,x){contexts.push(x);return new Promise(()=>{});}};const c=await open(config(s));for(let i=0;i<16;i++)await c.openChannel();[...state.sockets][0].destroy();await entered.promise;await until(()=>contexts.length===17);assert.equal(contexts.length,17);await c.close();assert(contexts.every(x=>x.signal.aborted));assert.deepEqual(warnings,[]);}finally{process.off('warning',warning);}
});
