...
type Request struct{URI,Scenario,Certificate string;Milliseconds int}
...
func main(){
 ...
 if r.Scenario=="unconfirmed"{check(ch.Confirm(false));confirmation,e=ch.PublishWithDeferredConfirm("","deadline-held",false,false,amqp.Publishing{Body:[]byte("before deadline close")});check(e)}
 ...
 if confirmation!=nil{out["confirmationDone"]=cancelled(confirmation.Done());out["confirmationAcked"]=confirmation.Acked()}
