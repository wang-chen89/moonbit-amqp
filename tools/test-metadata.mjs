...
await test('default identity factory returns independently mutable identity tables',async()=>{
 const a=newConnectionProperties(),b=newConnectionProperties();assert.deepEqual(a,{product:'moonbit-amqp',version:'0.22.0',platform:'moonbit'});a.product='changed';assert.equal(b.product,'moonbit-amqp');assert(!Object.hasOwn(b,'capabilities'));
});
