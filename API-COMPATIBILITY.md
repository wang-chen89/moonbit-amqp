...
“有入口”仅指已有对应操作及所注明的有限验证，不代表完整兼容。当前优先差距包括：完整 context/Go channel 通知语义、完整 TLS 状态/网络截止时间、完整自定义策略/关闭期限与恢复控制交错，以及完整恢复/长期/多版本/性能验证。
...
0.24 提供自定义 ConnectionRecovery 与默认委托：22 本地组、10 peer/12 broker 已测结果一致；关闭已失效资源的清理差异单列，见 [自定义恢复决策](CONNECTION-STRATEGY.md)。
...
| `DefaultConnectionRecovery.OnConnectionClose` | DefaultConnectionRecovery.onConnectionClose(connection, error, context) | 22 本地组、10 peer/12 broker 已测状态和结果一致；已有失效资源的显式关闭清理差异单列，非 Go goroutine/通知等价 |
| `DefaultConnectionRecovery.OnChannelClose` | DefaultConnectionRecovery.onChannelClose(channel, error, context) | 22 本地组、10 peer/12 broker 已测状态和结果一致；已有失效资源的显式关闭清理差异单列，非 Go goroutine/通知等价 |
