...
[API-COMPATIBILITY.md](API-COMPATIBILITY.md) 逐项映射固定参考库的 106 个公开名函数/方法声明，并给出可用对应接口或缺口。该数字不是独立能力数或完成比例，不包括所有结构字段、常量和接口。仍缺消费者独立取消、确认事件/序号/独立等待句柄、URI/自定义传输、连接元数据以及恢复查询/策略的完整接口与行为验证。
