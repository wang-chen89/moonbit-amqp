...
上述历史能力的计数保留各版本证据范围；0.19 当前重跑范围是完整本地 verify、自定义传输/元数据/URI 对照、RabbitMQ 基础、原生确认及双向流式正文回归。其余 14 份历史原生/broker 报告并非当前源码重新执行，详见 evidence/transport-upgrade.json。

## 仍需完善

[API-COMPATIBILITY.md](API-COMPATIBILITY.md) 逐项映射固定参考库的 106 个公开名函数/方法声明，并给出可用对应接口或缺口。该数字不是独立能力数或完成比例，不包括所有结构字段、常量和接口。仍缺完整 context/Go channel 通知语义、完整网络截止时间/传输平台验证、完整 TLS 状态以及恢复查询/策略的完整接口与行为验证。
