...
上述历史能力的计数保留各版本证据范围；0.18 当前重跑范围是完整本地 verify、元数据和 URI 对照、RabbitMQ 基础与原生确认回归。其余历史原生/broker 报告并非当前源码重新执行，详见 evidence/metadata-upgrade.json。

## 仍需完善

[API-COMPATIBILITY.md](API-COMPATIBILITY.md) 逐项映射固定参考库的 106 个公开名函数/方法声明，并给出可用对应接口或缺口。该数字不是独立能力数或完成比例，不包括所有结构字段、常量和接口。仍缺完整 context/Go channel 通知语义、自定义传输、完整 TLS 状态以及恢复查询/策略的完整接口与行为验证。
