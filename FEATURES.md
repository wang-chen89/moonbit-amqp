...
| 发送端流式正文 | UInt64 声明长度、同步/异步源、逐帧发送、可等待 drain、通道顺序、取消/超时、不重放 | 6 核心组、22 故障组、7 真实 broker 组，含 3 个 Go 大消息内容对照；接收/退回仍组装且上限 8 MiB，CLI stdin 仍 1 MiB |
...
尚缺上游恢复/API 全量边界、外部拓扑与并发恢复的完整验证、更多真实身份提供器与认证失败策略验证、流式接收、完整客户端扩展和生产规模背压压力验证。Node 每通道只允许一个 RPC，确认发布可并行。未测代表性吞吐、长时间运行、不同操作系统网络宿主或多版本 broker。JS/Wasm-GC 核心均通过 115 项；Node 网络实测平台为 Windows 24 + WSL RabbitMQ。方法层是线路格式校验，不执行全部域断言、保留字段和 broker 业务规则。
